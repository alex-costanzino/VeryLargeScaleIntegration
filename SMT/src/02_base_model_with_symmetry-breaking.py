...
def symmetry_model(width: int, ICs_number: int, IC_widths: list, IC_heights: list) -> tuple:
    '''
    Implement the base model with symmetry-breaking constraints for the 2d strip 
    packing problem.
    ...
    max_height_index = int(np.argmax(np.asarray(IC_heights)))
    ...
    # 4.1 Hard constraint: the IC with the highest height shall be placed in the origin.
    origin = [And(x[max_height_index] == 0, y[max_height_index] == 0)]
    
    # 4.2 There shall be an higher density on the left side of the board.
    density = [sum([If(x[i] <= width // 2, IC_widths[i]*IC_heights[i], 0) for i in range(ICs_number)]) >= 
               sum([If(x[i] > width // 2, IC_widths[i]*IC_heights[i], 0) for i in range(ICs_number)])]
    
    # Note: it's faster calculate the area when needed, in-place.
    
    constraints += (origin + density)
...
def main():
    ...
    for i in trange(1, 41):
        width, ICs_number, IC_widths, IC_heights = read_instance(ins + str(i) + '.txt') # Read the instance.
        solution = symmetry_model(width, ICs_number, IC_widths, IC_heights) # Pass the instance to the model.
